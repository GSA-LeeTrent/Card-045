DELIMITER $$
CREATE DEFINER=`HRDWCORPDATA`@`%` FUNCTION `getOrgByRoutingKey`(routingKey varchar(4), orgIdAssignedToQmsUser int) RETURNS int(11)
BEGIN
    declare orgId int; 
	...
	IF routingKey IS NOT NULL AND LENGTH(routingKey) > 0 THEN
